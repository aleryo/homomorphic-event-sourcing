  ...
  void $ run (read port) (serve devPetStoreApi $ enter (runServer store) handler)
    where
      ...
      action act =  ask >>= \ st -> do
        liftIO $ putStrLn $ "handling " <> show act
        ...
        case res of
          Just output -> do
            liftIO $ putStrLn $ "returning " <> show output
            return output
          Nothing     -> do
            liftIO $ putStrLn $ "returning error 400"
            throwError $ err400 { errBody = LBS.fromStrict $ encodeUtf8 $ pack $ "failed to " <> show act }
